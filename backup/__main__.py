...
dayEndings = {1: "st", 2: "nd", 3: "rd", 21: "st", 22: "nd", 23: "rd", 31: "st"}
...
def backup(
    ...
    t = datetime.utcfromtimestamp(cfg["lastBackupTime"])
    print(
    ...
    with Progress(console=terminal) as progress:
        ...
        backupBar = progress.add_task("[bold]Backing files up [/bold]", total=totalSize)
        ...
        for f in iterateFiles(cfg["paths"], cfg["excludes"]):
            if os.access(f["src"], os.R_OK):
                ...
                size += sz
                progress.log(updateInfo(f["src"], totalFiles, totalSize, files, size))
                try:
                    if cfg["useZip"]:
